!function(w){
	var DatePicker = function () {
		var getDom = function (i) {return document.getElementById(i)},
		...
		var init = function (n, config) {
			...
			this.config = config;
			this.D = new Date;
			this.el = getDom(config.inputId);
			this.el.key = this.n+'DatePicker';
			this.update();
			this.bind();
		}
		init.prototype = {
			update : function (y, m) {
				...
				fn = function (a, b) {return '<td key="'+_this.n+'DatePicker" class="noborder hand" onclick="'+_this.n+'.update('+a+')">'+b+'</td>'},
				_html = '<table cellpadding=0 cellspacing=2>';
				y && D.setYear(D.getFullYear() + y);
				m && D.setMonth(D.getMonth() + m);
				var year = D.getFullYear(), month = D.getMonth() + 1, date = D.getDate();
				for (var i=0; i<week.length; i++) con.push('<td key="'+this.n+'DatePicker" class="noborder">'+week[i]+'</td>');
				for (var i=0; i<D._fd(); i++ ) con.push('<td key="'+this.n+'DatePicker" class="noborder">&nbsp;</td>');
				...
				_html += '<tr>'+fn("-1, null", "<<")+fn("null, -1", "<")+'<td key="'+this.n+'DatePicker" colspan=3 class="strong">'+year+'/'+month+'/'+date+'</td>'+fn("null, 1", ">")+fn("1, null", ">>")+'</tr>';
			...
			show : function () {
				...
				s['left'] = is['left'] = this.el.offsetLeft + 'px';
				s['top'] = is['top'] = this.el.offsetTop + this.el.offsetHeight + 'px';
			...
			bind : function () {
				...
				if (t.getAttribute("key") != _this.n+'DatePicker' && t.key != _this.n+'DatePicker') {_this.hide()} else {_this.show()}
				})
			},
			createBox : function (html) {
				...
				box.key = this.n+'DatePicker';
				box.innerHTML = html;
				//document.body.appendChild(box);
				//document.body.appendChild(mask);
				this.el.parentNode.appendChild(box);
	...
	w.datePicker = function(n, config){
        return new DatePicker(n, config)
    }
}(window)