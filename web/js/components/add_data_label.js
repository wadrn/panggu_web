!function(w){
    ...
    Labedata.prototype.inithtml = function(){
        var data = this.data
        var html = '<div class="oz" id="_Labedata">' + this.labelbtnhtml() + this.labelconhtml() +this.labelselectedhtml() +'</div>'
        return html
    }
    ...
    Labedata.prototype.labelbtnhtml = function(){
        ...
        return '<div style="width: 160px;height: 390px;border-right:2px solid #eee;">' + html + '</div>'
    }
    ...
    Labedata.prototype.labelconhtml = function(){
        ...
        var html = '<div style="width:200px;height: 390px;position: absolute;top: 10px; left: 190px; border-right: 2px solid #eee;"><div class="mb20">' + this.labeltypehtml(data[this.curr])(arrdata,curdata) + '</div>'+nextbtn+'</div>'
        return html
    }
        ...
        obj['checkbox'] = function (data,cur){
            ...
            for(var i in data){
                if(cur.indexOf(data[i].id) >- 1){
                    html += '<div class="checkbox" style=" margin-bottom: 5px;"><label><input class="_checkbox" type="checkbox" name="'+_this.curr+'" value="'+data[i].id+'" checked/> '+data[i].detail+'</label></div>'
                }else{
                    html += '<div class="checkbox" style=" margin-bottom: 5px;"><label><input class="_checkbox" type="checkbox" name="'+_this.curr+'" value="'+data[i].id+'"/> '+data[i].detail+'</label></div>'
                }
            }
            return html;
        }
    ...
    Labedata.prototype._labedata =function(data){
        var html = ''
        for(var i in data){
            html += '<a class=" v btn blue" style="float: left; margin-right: 10px;margin-bottom: 10px; padding: 5px;border-radius: 3px">'+data[i]+'</a>'
        }
        return html
    }
